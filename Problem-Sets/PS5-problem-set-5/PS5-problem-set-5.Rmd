...
Each row represents one monthly CO2 measurement taken at the Mauna Loa Observatory.
The columns show the year, month, and the CO2 concentration (ppm) recorded for that month.
...
I found 12 of these using filter(co2 < 0) and changed them to NA with mutate(co2 = ifelse(co2 < 0, NA, co2)).
This helps clean up the data so only real CO2 values are left. And they should be 313.2 not -99.99 now
...
This shows the data covers about 60+ years of monthly CO2 readings.
---
...
It started around 315 ppm in 1958 and has gone up to over 400+ ppm by 2025.
...
hadcrut_long <- hadcrut_long %>%
  mutate(temp_anomaly = ifelse(temp_anomaly <= -9, NA, temp_anomaly)) %>%
...
#### **1.2.11 Comment the plot. How has the global temperature evolved through last 170 years?**
...
This way, each CO2 measurement can be matched with the global temperature anomaly from the same month and year.
...
merged_data
```
I used a left join to merge them by year and month. I kept all CO2 rows because the CO2 data is complete, but some temperature months might be missing. This way, we still keep every CO2 record and just get NA for months without temperature data.
...
merged_data %>%
  ...
  scale_color_gradientn(colors = heat.colors(10)) +
  labs(
    title = "CO2 vs Temperature Anomaly by Decade with Trend",
    x = "CO2 concentration (ppm)",
    y = "Temperature anomaly (C above pre-industrial)",
    color = "Decade"
  )
...
merged_long <- merged_temp %>%
  pivot_longer(c(surface, troposphere),
               names_to = "type",
               values_to = "anomaly") %>%
  mutate(type = ifelse(type == "troposphere", "lower troposphere", type))
...
So usually I check the course book and lecture notes first, then try things out in R. If I get stuck, I usually ask my friends.

---

### **3.2 How long time it took for you to do this homework?**

It took me 5-7 hours to complete this homework.

---

### **3.3 Which tasks were easy? Which tasks were the most challenging ones? What is most confusing? Please elaborate!**

The easy parts were loading and cleaning the data. The hardest part was merging and reshaping because of small details like missing years. Plotting was fun once everything worked.
