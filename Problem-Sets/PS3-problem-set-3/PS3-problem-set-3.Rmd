...
head(data, 5)
```
The dataset looks good. It shows country names, regions, life expectancy, GDP, and population values.
Some columns like GDP have missing values (NA), but overall the data looks clean and readable.
...
```
Columns like `name`, `region`, and the population columns (only 1 NA total) look good.
But `GDP_PC1960`, `GDP_PC2019`, and `le1960` have quite a few missing values, so they might need cleaning.
...
plot(x, y,
     main = "Life Expectancy (1960) vs % Growth to 2019 — by Country",
     xlab = "Life Expectancy in 1960 (years)",
     ylab = "LE Growth from 1960 to 2019 (%)",
     pch = 19,       
     cex = 0.9,      
     col = "dodgerblue")
...
```
50% of flights are delayed by more than 10 minutes
...
# three slowest (increasing speed)
flights_to_LAS %>%
  arrange(speed_mph) %>%
  select(year, month, day, dep_time, carrier, dep_delay, speed_mph) %>%
  head(3)
...
flights_to_LAS %>%
  ...
  summarise(daily_flights = n(), .groups = "drop") %>%
  summarise(max_daily = max(daily_flights),
            min_daily = min(daily_flights))
```

---

#### **3.3.7 If you did the previous question correctly, you see that one of the carriers is American Airlines (AA).**
...
```{r}
aa_position <- avg_delay_by_carrier %>% filter(carrier == "AA")
aa_position
```
...
#### **It took me around 8-9 Hours to complete the PS3 hehe**
